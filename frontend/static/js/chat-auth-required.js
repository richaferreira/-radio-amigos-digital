(()=>{
  ...
  function fixHero(){
    ...
    if(!hero)return;
    hero.src='/static/img/hero-radio-final.jpg?v=20260825-8';
  ...
  function installOnlineCounter(){
    ...
    if(typeof socket!=='undefined'){
      socket.on('online_count',data=>{
        ...
        $('#siteOnlineCount')&&($('#siteOnlineCount').textContent=count);
        $('#navOnlineCount')&&($('#navOnlineCount').textContent=count);
      });
  ...
  function configureTextField(el,logged,placeholder){
    if(!el)return;
    el.disabled=false;
    el.readOnly=!logged;
    el.classList.toggle('chat-auth-locked',!logged);
    if(placeholder)el.placeholder=placeholder;
    if(!logged)el.setAttribute('aria-disabled','true');else el.removeAttribute('aria-disabled');
  }
  ...
  function applyAccess(){
    ...
    configureTextField($('#homeChatInput'),logged,logged?'Digite sua mensagem...':'Clique aqui para entrar ou criar sua conta...');
    configureTextField($('#chatInput'),logged,logged?'Digite sua mensagem...':'Clique aqui para entrar ou criar sua conta...');
  ...
  async function verify(){
    ...
    const current=token();
    if(!current){verifiedUser=null;applyAccess();return null}
    checking=true;
    try{
      const r=await fetch('/api/auth/me',{headers:{Authorization:`Bearer ${current}`}});
      if(!r.ok)throw new Error();
      verifiedUser=await r.json();
    }catch{
  ...
  function blockAndLogin(event){
    if(verifiedUser)return false;
    const entry=wantsChat(event.target);
    if(!entry||event.target.closest('#radAuthModal'))return false;
    event.preventDefault();
    event.stopImmediatePropagation();
    if(!token())openModal();
    else verify().then(user=>{if(!user)openModal();else if(entry.matches?.('a[href="#chat"],.nav-chat'))location.hash='#chat'});
    return true;
  }

  document.addEventListener('pointerdown',blockAndLogin,true);
  document.addEventListener('click',blockAndLogin,true);
  ...
  document.addEventListener('submit',event=>{
    if(!event.target.matches('#homeChatForm,#chatForm,#chatSongRequestForm')||verifiedUser)return;
    event.preventDefault();
    event.stopImmediatePropagation();
    openModal();
  },true);
  ...
  document.addEventListener('click',event=>{
    const protectedControl=event.target.closest('.reaction-picker-toggle,.composer-emoji-toggle,[data-compose-emoji],#homeSongRequestBtn,#fullSongRequestBtn,.home-react-btn,.community-react-btn,.react-btn,[data-summary-emoji]');
    if(protectedControl&&!verifiedUser){event.preventDefault();event.stopImmediatePropagation();openModal()}
  },true);

  window.addEventListener('hashchange',()=>{
    if(location.hash==='#chat'&&!verifiedUser)verify().then(user=>{if(!user)openModal()});
  });
  ...
  installModal();
  verify();
  setTimeout(applyAccess,250);
  setTimeout(applyAccess,900);
  setInterval(verify,120000);
  loadExtra('/static/js/media-library.js?v=20260825-9','mediaLibraryScriptV3');
})();
