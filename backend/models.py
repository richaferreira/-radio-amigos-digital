...
class User(db.Model):
    ...
    badge_links = db.relationship(
        "UserBadge",
        foreign_keys="UserBadge.user_id",
        cascade="all, delete-orphan",
        lazy=True,
    )
    chat_style_record = db.relationship(
        "UserChatStyle",
        foreign_keys="UserChatStyle.user_id",
        cascade="all, delete-orphan",
        uselist=False,
        lazy=True,
    )
    ...
    def community_profile(self):
        ...
        for link in self.badge_links:
            if link.badge and link.badge.active:
                badges.append(link.badge.to_dict())
        style = self.chat_style_record.to_dict() if self.chat_style_record else dict(DEFAULT_CHAT_STYLE)
        return {"badges": badges, "chat_style": style}

    def to_dict(self):
        profile = self.community_profile()
        return {
            "id": self.id,
...
class BadgeDefinition(db.Model):
    ...
    created_at = db.Column(db.DateTime(timezone=True), default=now_utc)
    user_links = db.relationship("UserBadge", backref="badge", cascade="all, delete-orphan", lazy=True)
...
class UserBadge(db.Model):
    ...
    badge_id = db.Column(db.Integer, db.ForeignKey("badge_definition.id"), nullable=False, index=True)
...
class UserChatStyle(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey("user.id"), unique=True, nullable=False, index=True)
    font_style = db.Column(db.String(20), default="default")
    text_color = db.Column(db.String(20), default="default")
    effect = db.Column(db.String(20), default="none")
    updated_by = db.Column(db.Integer, nullable=True)
    updated_at = db.Column(db.DateTime(timezone=True), default=now_utc, onupdate=now_utc)

    def to_dict(self):
        return {
            "font_style": self.font_style or "default",
            "text_color": self.text_color or "default",
...
class StaffNotification(db.Model):
    ...
    kind = db.Column(db.String(30), nullable=False, default="info", index=True)
...
class ChatMessage(db.Model):
    ...
    def to_dict(self):
        ...
        profile = user.community_profile() if user else {"badges": [], "chat_style": dict(DEFAULT_CHAT_STYLE)}
        return {
            "id": self.id,
            "user_id": self.user_id,
            "nickname": user.display_name if user else self.nickname,
            "username": user.username if user else "",
            "role": user.role if user else self.role,
            "message": self.message,
            "deleted": self.deleted,
            "created_at": self.created_at.isoformat(),
            "reactions": {r.emoji: r.count for r in self.reactions},
            "badges": profile["badges"],
            "chat_style": profile["chat_style"],
        }
...
class PollOption(db.Model):
    ...
    def to_dict(self):
        return {"id": self.id, "label": self.label, "votes": self.votes}
